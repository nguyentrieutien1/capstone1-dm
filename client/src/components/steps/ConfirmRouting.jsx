/* eslint-disable jsx-a11y/alt-text */
import { Button, Typography } from "antd";
import React from "react";
import "./style.scss";
import { useSelector } from "react-redux";

const ConfirmRouting = (props) => {
  const {
    booked,
    setCurrent,
    handleContinueConfirmRouteStep,
  } = props;
  const scheduleById = useSelector(
    (state) => state.ScheduleReducer.scheduleById
  );


  return (
    <div className="confirm-route">
      <div className="confirm-route__top">
        <Typography.Title level={4}>xác nhận lộ trình đi</Typography.Title>
        <p className="confirm-route__top--price">
          {scheduleById.price.toLocaleString("it-IT", { style: "currency", currency: "VND", })}
        </p>
        <div className="confirm-route__top--group">
          <p><img src="data:image/png;base64," />{Date(scheduleById.startTime).slice(15, 21)} - {scheduleById.departureAddress.name}</p>
          <span className="confirm-route__top--distance">Khoảng cách: {scheduleById.distance}</span>
          <p><img src="data:image/png;base64," />{Date(scheduleById.endTime).slice(15, 21)} - {scheduleById.destinationAddress.name}</p>
        </div>
      </div>
      <div className="confirm-route__group">
        <div className="confirm-route__bottom">
          <p>Danh sách ghế đã chọn:</p>
          <div className="confirm-route__bottom__list">
            {booked.length > 0 && booked.map(item => {
              return <p key={item.id}>{item.name}</p>
            })}
          </div>
        </div>
        <div>
        <Button 
          className="confirm-route__group--back"
          onClick={() => setCurrent(0)}
        >Quay lại</Button>
        <Button
          className="confirm-route__group--continue"
          onClick={handleContinueConfirmRouteStep}
        >Tiếp tục</Button>
        </div>
      </div>
    </div>
  );
};

export default ConfirmRouting;
