...
export const CreateNewScheduleAction = (scheduleData) => async (dispatch) => {
  try {
    ...
    openNotificationWithIcon(`error`, `Tạo lịch trình thất bại. Vui lòng thử lại sau`);
    console.log(err);
  }
}
...
export const GetDetailBusInfoAction = (scheduleId) => async (dispatch) => {
  try {
    const result = await ManageBusInfoServices.GetDetailScheduleTicket(scheduleId);
...
export const UpdateBusInfoAction = (scheduleId, scheduleData) => async (dispatch) => {
  try {
    const result = await ManageBusInfoServices.UpdateDetailSchedule(scheduleId,scheduleData);
    if (result.status === 200) {
      openNotificationWithIcon(`success`, `Cập nhật thông tin lịch trình thành công !!!`);
    }
  } catch (err) {
    openNotificationWithIcon(`error`, `Cập nhật thông tin lịch trình thất bại. Vui lòng thử lại sau !!!`);
    console.log(err);
  }
}
